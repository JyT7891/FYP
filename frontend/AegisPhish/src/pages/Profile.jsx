...
export default function Profile() {
  ...
  const checkEmailExists = async (email) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/auth/check-email?email=${encodeURIComponent(email)}`, { headers });
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      // If email was changed, show warning and refresh user data
      if (emailChanged) {
        setEmailChangedWarning(true);
        showToast("Email changed! A new verification email has been sent to your new address.", "warning");
  ...
  const handleRemoveAvatar = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/user/avatar", { method: "DELETE", headers });
  ...
  return (
    <>
      ...
      <div className="p-8 space-y-6 w-full max-w-4xl mx-auto">
        {/* Email Change Warning Banner */}
        {emailChangedWarning && !user?.email_verified && (
          <div className="rounded-xl border border-orange-500/30 bg-orange-500/10 p-4">
            <div className="flex items-start gap-3">
              <span className="text-xl">⚠️</span>
              <div className="flex-1">
                <p className="text-sm font-semibold text-orange-400">Email Change Requires Verification</p>
                <p className="text-xs text-gray-300 mt-1">
                  Your email has been changed to <span className="text-orange-400 font-mono">{profile.email}</span>. 
                  A verification email has been sent to your new address. Please check your inbox and click the verification link.
                </p>
                <button
        ...
        {/* Email Verification Status (only show if not verified and no recent change) */}
        {!user?.email_verified && !emailChangedWarning && (
          <div className="rounded-xl border border-orange-500/30 bg-orange-500/10 p-4">
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="divide-y divide-teal-500/10">
              {recentActivity.map((scan, i) => (
                <div key={i} onClick={() => navigate(`/scan/${scan._id}`)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-sm font-mono text-gray-300 truncate group-hover:text-teal-400 transition">{scan.url}</p>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-600">
                      <span>{timeAgo(scan.scanned_at)}</span>
                      <span>•</span>
                      <span className={scan.prediction === "Legitimate" ? "text-teal-400" : scan.prediction === "Suspicious" ? "text-orange-400" : "text-red-400"}>
                        {scan.prediction}
                      </span>
                      <span>•</span>
