// Admin reports page for reviewing and managing user-submitted phishing reports
...
export default function AdminReports() {
  ...
  // Currently selected report for viewing details
  const [selectedReport, setSelectedReport] = useState(null);
  // Controls authority email modal visibility
  const [showAuthorityModal, setShowAuthorityModal] = useState(false);
  // Selected authority email address
  const [authorityEmail, setAuthorityEmail] = useState("");
  // Optional message to include in authority email
  ...
  // Fetch all pending reports from backend
  ...
  // Mark report as dismissed/invalid
  ...
  // Send phishing report to external cybersecurity authority
  ...
  return (
    <>
      ...
      <div className="p-8">
        {/* Loading / empty / list state rendering */}
        {loading ? (
          ...
          <div className="space-y-3">
            {reports.map((report) => (
              <div
                key={report._id}
                className="rounded-xl border border-purple-500/20 bg-gradient-to-b from-[#0f0f23] to-[#0a0a1a] p-4"
              >
            ...
            <div className="space-y-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  {selectedReport.note || "No additional notes"}
                </p>
              </div>
            </div>
      ...
      {showAuthorityModal && selectedReport && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-[#0f0f23] border border-yellow-500/30 rounded-xl p-6 max-w-lg w-full shadow-2xl">
            <h3 className="text-base font-semibold mb-2 text-yellow-400">
              Report to Cybersecurity Authorities
            </h3>
            <p className="text-xs text-gray-400 mb-4">
              Forward this phishing URL to cybersecurity authorities for
            ...
            <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-3 mb-4">
              ...
              <div className="text-xs text-gray-400 space-y-1">
                <p>
                  ...
                  {authorityEmail || "[Selected Authority]"}
                </p>
                <p>
                  <span className="text-gray-500">Subject:</span> Phishing URL
                  Report - AegisPhish
                </p>
                <p>
                ...
                <p className="pl-4">
                  A phishing URL has been detected and reported through
                  AegisPhish:
                </p>
                <p className="pl-4 font-mono text-red-400 break-all">
                  {selectedReport.url}
                </p>
                {authorityMessage && (
                  <p className="pl-4 mt-2">
                    Additional notes: {authorityMessage}
                  </p>
                )}
                ...
                <p className="pl-4 text-gray-500">
                  Sent via AegisPhish Phishing Detection System
                </p>
              </div>
            </div>
