...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (!form.password) e.password = "Password is required.";
      ...
      e.password = "Password must contain at least one special character (!@#$%^&* etc.).";
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/auth/verify-registration", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: userId,
  ...
  const handleResendCode = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://127.0.0.1:8000/api/auth/resend-verification-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
