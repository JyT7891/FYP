...
// Circular risk gauge displaying score and prediction status
...
// Horizontal bar visualization for SHAP feature importance
...
// Main Scan URL page component for phishing detection input and results
export default function ScanURL() {
  ...
  // Build authenticated API request headers
  const getHeaders = () => {
  ...
  // Validate and normalize URL input before scan request
  ...
  // Re-run scan for the current or last scanned URL
  ...
  // Submit phishing report for the scanned URL
  ...
  const shapEntries = result?.shap_values
    ? Object.entries(result.shap_values)
        ...
        .slice(0, 10)
    : [];
  // Maximum SHAP value used for scaling visualization
              ...
              <div className="rounded-xl border border-teal-500/20 bg-gradient-to-b from-[#0a192f] to-[#06111f] p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    {
                      label: "HTTPS",
                      value: result.url.startsWith("https") ? "✓ Yes" : "✗ No",
                      color: result.url.startsWith("https")
                        ? "text-teal-400"
                        : "text-red-400",
                    },
                    {
                      label: "Dots",
                      value: (result.url.match(/\./g) || []).length,
                    },
                    {
                      label: "Hyphens",
                      value: (result.url.match(/-/g) || []).length,
                    ...
                    {
                      label: "Digits",
                      value: (result.url.match(/[0-9]/g) || []).length,
                    },
                    {
                      label: "Slashes",
                      value: (result.url.match(/\//g) || []).length,
                    ...
                    {
                      label: "Has @",
                      value: result.url.includes("@") ? "✗ Yes" : "✓ No",
                      color: result.url.includes("@")
                        ? "text-red-400"
                        : "text-teal-400",
                    },
                    {
                      label: "TLD",
                      value: result.url.split(".").pop()?.split("/")[0] ?? "—",
                    },
                  ].map((item) => (
                    <div
                      key={item.label}
        ...
        {!result && !scanning && !error && !validationError && (
          <div className="rounded-xl border border-teal-500/10 border-dashed p-12 text-center">
            ...
            <p className="text-gray-400 text-sm font-medium">
              Enter a URL above to run a full analysis
            </p>
            <p className="text-gray-600 text-xs mt-2">
              Results include ML prediction, risk score, SHAP breakdown, and
