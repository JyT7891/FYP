...
export default function AdminScans() {
  ...
  const fetchScans = async () => {
    try {
      ...
      // Calculate stats
      const phishing = scansData.filter(s => s.prediction === "Phishing").length;
      const suspicious = scansData.filter(s => s.prediction === "Suspicious").length;
      const legitimate = scansData.filter(s => s.prediction === "Legitimate").length;
      const totalRisk = scansData.reduce((sum, s) => sum + (s.risk_score || 0), 0);
      ...
      setStats({
        ...
        suspicious,
        legitimate,
        avgRisk: scansData.length > 0 ? (totalRisk / scansData.length).toFixed(1) : 0,
      });
    } catch (error) {
  ...
  const isAllSelected = filteredScans.length > 0 && filteredScans.every(scan => selectedScans.has(scan._id));
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${selectedIds.length} scan(s)? This action cannot be undone.`)) return;
    
    setDeleting(true);
    try {
      const response = await fetch("http://127.0.0.1:8000/admin/scans/bulk-delete", {
        method: "POST",
        headers,
  ...
  const exportToCSV = () => {
    setExporting(true);
    try {
      const csvHeaders = ["URL", "Prediction", "Risk Score (%)", "Reasons", "Scanned At", "User Email"];
    ...
    <>
      <header className="border-b border-purple-500/20 px-6 py-4 bg-[#0f0f23]/80 backdrop-blur sticky top-0 z-10">
        <div>
          ...
          <p className="text-xs text-gray-500">View and manage all URL scans across the system</p>
        </div>
      </header>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          ...
          <div className="rounded-xl border border-purple-500/20 bg-gradient-to-b from-[#0f0f23] to-[#0a0a1a] p-4">
            ...
            <p className="text-2xl font-bold text-teal-400">{stats.legitimate}</p>
          </div>
          <div className="rounded-xl border border-purple-500/20 bg-gradient-to-b from-[#0f0f23] to-[#0a0a1a] p-4">
            ...
            <table className="w-full text-left">
              <thead className="border-b border-purple-500/20 bg-[#0f0f23]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-xs text-gray-500 uppercase">URL</th>
                  <th className="px-4 py-3 text-xs text-gray-500 uppercase">Prediction</th>
                  <th className="px-4 py-3 text-xs text-gray-500 uppercase">Risk</th>
                  <th className="px-4 py-3 text-xs text-gray-500 uppercase">User</th>
                  <th className="px-4 py-3 text-xs text-gray-500 uppercase">Scanned At</th>
                  <th className="px-4 py-3 text-xs text-gray-500 uppercase">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-purple-500/10">
                {filteredScans.map((scan) => (
                  <tr key={scan._id} className="hover:bg-white/[0.02] transition">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm font-mono text-gray-300 truncate max-w-md" title={scan.url}>
                        {scan.url}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-sm font-mono ${
                        ...
                        scan.risk_score >= 35 ? "text-orange-400" :
                        "text-teal-400"
                      }`}>
                        {scan.risk_score}%
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs text-gray-400" title={scan.user_email}>
                        {scan.user_email || "Anonymous"}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-[#0f0f23] border border-purple-500/30 rounded-xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-purple-400">Scan Details</h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm font-mono text-gray-300 break-all">{selectedScan.url}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className={`text-sm px-3 py-1 rounded-full inline-block ${
                    selectedScan.prediction === "Legitimate" ? "bg-teal-500/10 text-teal-400" :
                    selectedScan.prediction === "Suspicious" ? "bg-orange-500/10 text-orange-400" :
                    "bg-red-500/10 text-red-400"
                  }`}>
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold ${
                    selectedScan.risk_score >= 70 ? "text-red-400" :
                    selectedScan.risk_score >= 35 ? "text-orange-400" :
                    "text-teal-400"
                  }`}>
              ...
              {selectedScan.reasons && selectedScan.reasons.length > 0 && (
                <div>
                  <p className="text-xs text-gray-500 mb-2">Detection Reasons</p>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-purple-500/20">
                <div>
                  ...
                  <p className="text-xs font-mono text-gray-500">{selectedScan._id}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">User</p>
                  <p className="text-xs font-mono text-gray-500">{selectedScan.user_email || "Anonymous"}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Scanned At</p>
                  <p className="text-xs text-gray-400">{formatDate(selectedScan.scanned_at)}</p>
                </div>
              </div>
            </div>
